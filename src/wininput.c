...
win_init_ctxmenu(void)
{
  ...
  AppendMenu(ctxmenu, MF_ENABLED, IDM_NEWTAB, "New tab\tCtrl+Shift+T");
  AppendMenu(ctxmenu, MF_ENABLED, IDM_KILLTAB, "Kill tab");
  AppendMenu(ctxmenu, MF_SEPARATOR, 0, 0);
  AppendMenu(ctxmenu, MF_ENABLED, IDM_PREVTAB, "Previous tab\tShift+<-");
  AppendMenu(ctxmenu, MF_ENABLED, IDM_NEXTTAB, "Next tab\tShift+->");
  AppendMenu(ctxmenu, MF_ENABLED, IDM_MOVELEFT, "Move to left\tCtrl+Shift+<-");
  AppendMenu(ctxmenu, MF_ENABLED, IDM_MOVERIGHT, "Next to right\tCtrl+Shift+->");
  AppendMenu(ctxmenu, MF_SEPARATOR, 0, 0);
