    ...
    if (select(highfd + 1, &fds, 0, 0, timeout_p) > 0) {
      for (Tab& t : win_tabs()) {
        ...
        if (child_p->pty_fd >= 0 && FD_ISSET(child_p->pty_fd, &fds)) {
          ...
          if (child_p->term->baud > 0) {
            uint cps = child_p->term->baud / 10; // 1 start bit, 8 data bits, 1 stop bit
