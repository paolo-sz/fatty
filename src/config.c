...
const config default_cfg = {
  ...
  fontfams : {{name : W(""), size : 0, weight : 400, isbold : false},
...
set_option(string name, string val_str, bool from_file)
{
  ...
  void *val_p = (char *)&cfg + options[i].offset;
  ...
  switch (type) {
    when OPT_STRING:
      strset((const char **)val_p, val_str);
      return i;
    when OPT_WSTRING: {
      wchar * ws;
      if (from_file)
...
#endif
      if (parse_colour(val_str, (colour *)val_p))
        return i;
    when OPT_COLOUR_PAIR: {
#ifdef debug_theme
      printf("set_option <%s> <%s>\n", name, val_str);
#endif
      colour_pair *pair = (colour_pair *)val_p;
...
copy_config(char * tag, config * dst_p, const config * src_p)
{
  ...
  for (uint i = 0; i < lengthof(options); i++) {
    ...
    if (!(type & OPT_LEGACY)) {
      ...
      void *dst_val_p = (char *)dst_p + offset;
      void *src_val_p = (char *)src_p + offset;
      switch (type & OPT_TYPE_MASK) {
        when OPT_STRING:
          strset((string *)dst_val_p, *(string *)src_val_p);
        when OPT_WSTRING:
          wstrset((wstring *)dst_val_p, *(wstring *)src_val_p);
        when OPT_INT:
  ...
  if (!file) {
    ...
    for (uint j = 0; j < file_opts_num; j++) {
      ...
      if (!(type & OPT_LEGACY)) {
        ...
        void *cfg_p = &file_cfg;
        void *val_p = (char *)cfg_p + options[i].offset;
        switch (type & OPT_TYPE_MASK) {
          when OPT_STRING:
...
apply_config(bool save)
{
  ...
  for (uint i = 0; i < lengthof(options); i++) {
    ...
    void *val_p = (char *)&file_cfg + offset;
    void *new_val_p = (char *)&new_cfg + offset;
    if (!(type & OPT_LEGACY)) {
      bool changed;
      switch (type & OPT_TYPE_MASK) {
        when OPT_STRING:
...
{
  auto terminfo_exists = [&](char * ti) -> bool {
    auto terminfo_exists_in = [&](char * dir, char * sub, char * ti) -> bool {
    ...
    if (support_wsl) {
      ...
      if (wslname) {
        ...
        wslroot = strdup("");
      bool ex = terminfo_exists_in(wslroot, const_cast<char *>("/usr/share/terminfo"), ti);
      free(wslroot);
      return ex;
    }
    else
...
enterfontlist(wchar * fn, int weight, wchar * style)
{
  ...
  if (found) {
    ...
    fontlist[fi].weights = newn(weight_t, 1);
...
fontenum(const ENUMLOGFONTW *lpelf, const NEWTEXTMETRICW *lpntm, DWORD fontType, LPARAM lParam)
{
  ...
  if (pdata->outer) {
    ...
    auto tagsplit = [&](wchar * fn, wstring style) -> wchar *
    {
#if CYGWIN_VERSION_API_MINOR >= 74
...
{
  string dir = (string)ctrl->context;
...
setup_config_box(controlbox * b)
{
  ...
  */
  ...
  (store_button_norm = ctrl_pushbutton(s, _("Store"), scheme_saver, (void *)""))
    ->column = 1;
  if (cfg.config_themes == 2) {
    ctrl_columns(s, 1, 100);  // reset column stuff so we can rearrange them
    ctrl_columns(s, 2, 75, 25);
    ...
    dheme->column = 0;
    //__ Options - Looks: store colour scheme
    (store_button_dark = ctrl_pushbutton(s, _("Store"), scheme_saver, (void *)"d"))
      ->column = 1;
  }
  ctrl_columns(s, 1, 100);  // reset column stuff so we can rearrange them
  ...
  ctrl_pushbutton(s, _("Color Scheme Designer"), url_opener, (void *)(W("http://ciembor.github.io/4bit/")))
    ->column = 1;
  ...
  ctrl_pushbutton(
    s, _("◄"), transparency_slider, const_cast<char *>("-")
  )->column = 1;
  ctrl_pushbutton(
    s, _("►"), transparency_slider, const_cast<char *>("+")
  )->column = 3;
