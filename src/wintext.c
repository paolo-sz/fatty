...
win_init_fontfamily(HDC dc, int findex)
{
  ...
  ff->bold_mode = cfg.bold_as_font ? BOLD_FONT : BOLD_NONE;
...
win_text(int x, int y, wchar *text, int len, cattr attr, cattr *textattr, ushort lattr, bool has_rtl)
{
 ...
 /* Convert to window coordinates */
  ...
  y = y * cell_height + PADDING + g_render_tab_height;
...
win_check_glyphs(wchar *wcs, uint num)
{
  ...
  HDC dc = GetDC(wnd);
  bool bold = (ff->bold_mode == BOLD_FONT) && (win_active_terminal()->curs.attr.attr & ATTR_BOLD);
