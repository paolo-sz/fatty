// wintext.c (part of FaTTY)
// Copyright 2015 Juho Peltonen
// Based on code from mintty 2008-22 Andy Koppe, 2015-2026 Thomas Wolff
 ...
 */
void
(term_indicate_ime)(struct term* term_p, bool open_native)
{
  TERM_VAR_REF(true)
 ...
 */
void
(win_text)(struct term* term_p, int tx, int ty, wchar *text, int len, cattr attr, cattr *textattr, ushort lattr, char has_rtl, char has_sea, bool clearpad, uchar phase)
{
  TERM_VAR_REF(true)
...
#define DRAW_DOWN  0x4
  ...
  HRGN clipr;
  auto setclipr = [&](int x, int y, int n)
  {
...
#define dh 0x51
    ...
    auto boxlines = [&](bool heavy, char x1, char y1, char x2, char y2, char x3, char y3)
    {
      auto boxscale= [&](int ref, char val) -> int
      {
        if (val >= dl) {
          if (val > dl)
      ...
      auto boxline = [&](int x1, int y1, int x2, int y2)
      {
        ...
        if (y3 < -2) {  // dashed lines
          ...
          RECT tmp_rect = {xi + x1, y0 + y1, xi + x2, y0 + y2};
...
dw_has_glyph(xchar xc, cattrflags attr)
{
  ...
  if (!ff->cached) {
    ...
    LOGFONTW lf = {font_height, 0, 0, 0, w, (BYTE)i, false, false,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        (BYTE)get_font_quality(), FIXED_PITCH | FF_DONTCARE,
        W("")};
...
wchar
(get_errch)(struct term* term_p, wchar *wcs, cattrflags attr)
{
  TERM_VAR_REF(true)
