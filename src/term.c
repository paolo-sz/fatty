...
(term_schedule_vbell)(struct term* term_p, int already_started, int startpoint)
{
  ...
  if ((term.in_vbell = ticks > 0))
    win_set_timer(vbell_cb, term_p, ticks);
}
...
void
(term_search_expand)(struct term* term_p, int idx)
{
  TERM_VAR_REF(true)
...
#define results_find_ge(...) (results_find_ge)(term_p, ##__VA_ARGS__)
static result
(results_find_ge)(struct term* term_p, int idx)
{
  TERM_VAR_REF(true)
...
#define results_find_le(...) (results_find_le)(term_p, ##__VA_ARGS__)
static result
(results_find_le)(struct term* term_p, int idx)
{
  TERM_VAR_REF(true)
...
result
(term_search_next)(struct term* term_p)
{
  TERM_VAR_REF(true)
...
result
(term_search_prev)(struct term* term_p)
{
  TERM_VAR_REF(true)
...
/*
   ...
   to its limit.
 */
#define scrollback_trim(...) (scrollback_trim)(term_p, ##__VA_ARGS__)
static void
(scrollback_trim)(struct term* term_p)
{
  TERM_VAR_REF(true)
...
static void
(scrollback_push)(struct term* term_p, uchar *line, int newrows)
{
  TERM_VAR_REF(true)
...
#define printsb(...) (printsb)(term_p, ##__VA_ARGS__)
static void
(printsb)(struct term* term_p, char * tag)
{
  TERM_VAR_REF(true)
...
(term_reflow)(struct term* term_p, int newrows, int newcols, bool quick_reflow)
{
  ...
  auto cursor_scroll = [&](termline *tl)
  {
 ...
 */
void
(term_clear_status)(struct term* term_p)
{
  TERM_VAR_REF(true)
 ...
 */
void
(term_switch_status)(struct term* term_p, bool status_line)
{
  TERM_VAR_REF(true)
...
(term_erase)(struct term* term_p, bool selective, bool line_only, bool from_begin, bool to_end)
{
  ...
  if (from_begin)
    start = (pos){y : line_only ? curs->y : top_y, x : 0, piy : 0, pix : 0, r : false};
  else
    start = (pos){y : curs->y, x : curs->x, piy : 0, pix : 0, r : false};

  if (to_end)
    end = (pos){y : line_only ? curs->y + 1 : bot_y, x : 0, piy : 0, pix : 0, r : false};
  else
...
struct emoji_seq {
  const wchar * efn;   // image filename
  void * buf;    // cached image
  int buflen;    // cached image
  echar chs[10]; // code points
  const char * name;   // short name in emoji-sequences.txt, emoji-zwj-sequences.txt
};
...
#define dont_debug_width_scaling
    ...
    */
    ...
    if (prevdirtyitalic) {
      ...
      win_text(term.cols, i, const_cast<wchar *>(W(" ")), 1, CATTR_DEFAULT, (cattr*)&CATTR_DEFAULT, line->lattr, false, false, true, 0);
    }
    if (firstdirtyitalic) {
      // clear overhang into left padding border
      win_text(-1, i, const_cast<wchar *>(W(" ")), 1, CATTR_DEFAULT, (cattr*)&CATTR_DEFAULT, line->lattr, false, false, true, 0);
    }
...
#define dont_debug_bidi_paragraphs
    ...
    */
    ...
    int maxtextlen = max((int)(line->size), 16);
...
# define debug_out_text
    ...
    auto out_text = [&](int x, int y, wchar *text, int len, cattr attr, cattr *textattr, ushort lattr, char has_rtl, char has_sea)
    {
      ...
      if (attr.attr & TATTR_EMOJI) {
        ...
        if (elen) {
          ...
          if (cfg.emoji_placement == EMPL_FULL && !overlaying)
            ...
            struct emoji * ee = (struct emoji *)&eattr.truefg;
