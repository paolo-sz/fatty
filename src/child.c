...
static char state = 0;
  ...
  auto term_log_char = [&](struct term* term_p, char c)
  {
    TERM_VAR_REF(true)
...
void
(child_close_log)(struct term* term_p)
{
  TERM_VAR_REF(true)

  char buf = '\e';
  term_log(&buf, 1);  // trigger term_log_flush();
}
  ...
  if (sync_level()) {
    ...
    if (cfg.tabbar) {
      ...
      if (in_tabs && *cfg.classname) {
        ...
        setenv(const_cast<char *>("FATTY_CLASS"), classname, true);
        free(classname);
      }
      else if (!in_tabs)
        setenv(const_cast<char *>("FATTY_CLASS"), "+", true);
    }
  }
}
