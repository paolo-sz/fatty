...
#define support_multiline_bidi
  ...
  if (autodir && line->lattr & LATTR_WRAPCONTD && scr_y > -sblines(term)) {
    ...
    bool brk = false;
    do {
      termline *prevline = fetch_line(term, term->disptop + y);
  ...
  if (autodir && !prevseldir && (line->lattr & LATTR_WRAPPED) && term->disptop + scr_y < 0) {
    ...
    while (autodir) {
      ...
      succline = fetch_line(term, term->disptop + y);
  ...
  if (!term_bidi_cache_hit(term, scr_y, line->chars, line->lattr, term->cols)) {
    ...
    if (autodir && rtl >= 0) {
      ...
      if (true) {  // limiting to prevseldir does not work
        ...
        int paray = scr_y;
        while ((paraline->lattr & LATTR_WRAPCONTD) && paray > -sblines(term)) {
          paraline = fetch_line(term, --paray);
          bool brk = false;
